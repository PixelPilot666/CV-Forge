...
- 已有 `$PROFILE`（`~/.cv-forge/master-profile.yaml`）→ 校验：
  ...
  ```
- 没有 → 先 `mkdir -p ~/.cv-forge`，从用户旧简历抽取（schema 见 `references/profile-schema.md`）：
  ```bash
...
### 3.5 缺口追问访谈（专家关键环节）
若发现 bullet 缺数字、或某 JD 要求只是「部分」匹配，按 `references/interview.md` **主动向用户提问**，挖出真实的规模/指标/成果，写回 `$PROFILE`（经用户确认）。只挖真实信息，绝不替用户编造。
...
```bash
mkdir -p "$OUT"
# 模板资源(resume.cls/*.sty/fonts/、照片)复制到 $OUT 后再编译（字体走相对路径）
cp -R "$SKILL/assets/templates/zh-classic/." "$OUT/"
...
把评审分数、对抗发现、迭代过程写入 `$OUT/match-report.md`（覆盖表 + ATS 命中 + 缺口 + 补强审计 + **独立评审结论**）。
