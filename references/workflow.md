...
- 有 `data/master-profile.yaml` → `validate_profile.py` 校验；按软提醒补全。
...
```bash
mkdir -p output/<dir>
cp -R assets/templates/zh-classic/. output/<dir>/      # 字体/cls/sty 就位(相对路径)
...
照片：profile `basics.photo` 为 true 时模板会引用 `images/`，需确保 `output/<dir>/images/` 下有照片（默认带 placeholder.jpg；用户可替换为自己的）。

## 6. 匹配报告

```bash
python3 scripts/ats_check.py examples/sample-jd.txt output/<dir>/resume.pdf
```

把命中表与覆盖表、缺口清单、补强审计合并写入 `output/<dir>/match-report.md`。
...
- profile 校验失败：先修 `data/master-profile.yaml` 再继续。
